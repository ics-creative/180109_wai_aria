...
Reactではなく他のJSライブラリの作り方を知りたい方にも、記事をすでに公開しています。Angularでの実装方法は記事「[AngularでWAI\-ARIA対応のタブ型UIを実装する方法 \- Qiita](https://qiita.com/clockmaker/items/2ccf50429a5465ea5a69)」を参考ください。Vue.js版は後日公開予定です。
...
対して、**全くアクセシビリティーを意識せずに設計した場合どうなるでしょう？** 次のサンプルは、`aria`属性や`role`属性を使わず、さらに`a`要素や`button`要素、`tabindex`属性など全く配慮しなかったものです。**ほとんど的外れな読み上げをしていて使い物になりません**。タブキーで操作不能でマウスでしか操作できません。
...
では、WAI-ARIAに対応したReactのサンプルを紹介します。GitHubにアップしているので、デモとソースコードをご覧ください。
...
WAI-ARIAの実装にはHTMLの正しいマークアップとJavaScriptの両方が必要です。が、実際にはウェブアクセシビリティーへの意識の高い人が実装している印象があるものの、JavaScriptエンジニアの界隈からはあまり興味をもたれている気がしません。
